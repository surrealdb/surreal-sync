...
- Invocation: two CLI runs — `full` (`--strategy sequential-snapshot`) then `incremental --incremental-from <t1>`. For MySQL, see [MySQL Legacy Full Sync](../mysql/legacy.md). For PostgreSQL trigger source, see [PostgreSQL Legacy Full Sync (Trigger)](../postgresql/legacy.md). For PostgreSQL wal2json, see [PostgreSQL wal2json Legacy Full Sync](../postgresql-wal2json/legacy.md).
