...
Apply into SurrealDB goes through the [`sync-transform`](transforms.md) framework (`--transforms-config` optional; omit for identity). Pre-existing `conversion_rules` still run before Pipeline stages. Each file is streamed by a long-lived `SourceDriver` (line reads poll into the apply window; there is no per-`batch-size` runtime restart barrier).
