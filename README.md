...
- **[MariaDB](docs/mariadb.md)**: Full and incremental sync via the same trigger-based source as MySQL
- **[MySQL/MariaDB (binlog)](docs/mysql-binlog.md)**: Triggerless binlog CDC — snapshot, stream, and resume via a checkpoint store (`--checkpoint-dir` or `--checkpoints-surreal-table`); restart the same command after stop or repoint
