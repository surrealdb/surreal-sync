.PHONY: test check fmt clippy build build-debug compile-embedder-examples embed-dependency-tree-gates clean clean-logs help install-tools install-hooks system-deps prebuild-test-images prepull-binlog-images prepull-mssql-image prepull-bigquery-image print-test-images services-info
...
# Pre-pull stock test images and pre-build the custom PostgreSQL (wal2json) test
# image so parallel test processes find them in Docker's layer cache instead of racing.
prebuild-test-images: prepull-binlog-images prepull-mssql-image prepull-bigquery-image
