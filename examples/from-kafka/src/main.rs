//! Custom in-process Kafka transforms (embedder example).
//!
//! Same flags as `surreal-sync from kafka`, but without the `from kafka`
...
/// Drop fields that must not leave the source VPC.
struct RedactPii;
