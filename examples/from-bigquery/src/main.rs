//! Custom in-process BigQuery transforms (embedder example).
//!
//! Same flags as `surreal-sync from bigquery`, but without the `from bigquery`
//! prefix. Uses SurrealDB 3 via `run::<Surreal3Sink>`:
//!
//! ```bash
//! cargo run -p surreal-sync-example-from-bigquery -- \
...
/// Drop columns that must not leave the warehouse.
struct RedactPii;
