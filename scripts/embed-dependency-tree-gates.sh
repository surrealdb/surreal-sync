...
for pkg in surreal-sync-kafka surreal-sync-snowflake surreal-sync-bigquery surreal-sync-json surreal-sync-runtime surreal-sync-mysql surreal-sync-mssql; do
...
# snowflake / bigquery / mysql must not pull kafka or neo4j clients
for pkg in surreal-sync-snowflake surreal-sync-bigquery surreal-sync-mysql surreal-sync-json surreal-sync-mssql; do
...
for pkg in surreal-sync-example-from-snowflake surreal-sync-example-from-bigquery surreal-sync-example-from-mysql-binlog surreal-sync-example-from-jsonl surreal-sync-example-from-mssql; do
  for deny in rdkafka neo4rs; do
    out=$(cargo tree -p "$pkg" -e normal,build -i "$deny" 2>&1 || true)
    if echo "$out" | rg -q "^${deny} "; then
      echo "FAIL: $pkg unexpectedly depends on $deny:"
...
if ! rg -n 'run::<Surreal3Sink>' examples/from-snowflake examples/from-bigquery examples/from-mysql-binlog examples/from-jsonl examples/from-kafka examples/from-mssql; then
  echo "FAIL: examples should monomorphize Surreal3Sink"
  exit 1
fi
if rg -n 'run::<Surreal2Sink>|Surreal2Sink' examples/from-snowflake examples/from-bigquery examples/from-mysql-binlog examples/from-jsonl examples/from-kafka examples/from-mssql; then
