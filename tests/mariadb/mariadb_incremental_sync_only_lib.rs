//! MariaDB incremental sync E2E test (thin wrapper).
//!
...
//! exercises MariaDB's `LONGTEXT`-backed JSON handling through the trigger audit
//! path (JSON_EXTRACT nesting) on the unified dataset's nested `metadata` column.
...
#[tokio::test]
async fn test_mariadb_incremental_sync_lib() -> Result<(), Box<dyn std::error::Error>> {
