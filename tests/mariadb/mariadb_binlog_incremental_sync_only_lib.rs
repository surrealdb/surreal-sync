//! MariaDB binlog incremental sync E2E test (thin wrapper).

use surreal_sync::testing::generate_test_id;
use surreal_sync::testing::mysql_binlog_e2e::run_binlog_incremental_e2e;
use surreal_sync::testing::shared_containers::{
    create_binlog_test_db, shared_mariadb_binlog, shared_surrealdb,
};

#[tokio::test]
async fn test_mariadb_binlog_incremental_sync_lib() -> Result<(), Box<dyn std::error::Error>> {
