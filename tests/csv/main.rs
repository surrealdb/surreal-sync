//! CLI e2e: `from csv --transforms-config` with an external mutate worker.
...
#[tokio::test]
async fn test_csv_cli_transforms_config_mutate() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let work_dir = format!(".test-csv-transforms-cli-{test_id}");
    std::fs::create_dir_all(&work_dir)?;

    let mut csv_file = NamedTempFile::new_in(&work_dir)?;
    writeln!(csv_file, "id,name,age")?;
    writeln!(csv_file, "1,Alice,30")?;
    writeln!(csv_file, "2,Bob,25")?;
    csv_file.flush()?;
    let csv_path = csv_file.path().to_string_lossy().to_string();
...
"#,
    ...
    assert_cli_success(&output, "CSV CLI with --transforms-config mutate");
