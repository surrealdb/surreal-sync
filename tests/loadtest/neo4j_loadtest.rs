//! Neo4j load testing integration test.
//!
//! This test demonstrates the populate -> sync -> verify workflow:
//! 1. Clean Neo4j and SurrealDB of any existing test data
//! 2. Populate Neo4j with deterministic test data using a fixed seed
//! 3. Run full sync from Neo4j to SurrealDB
...
#[tokio::test]
async fn test_neo4j_loadtest_small_scale() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tracing::info!("Cleaning up Neo4j labels: {:?}", all_table_names);
    ...
    for table_name in &table_names {
        // Neo4j stores all IDs as strings, so we need to tell the verifier to use string IDs
        let mut verifier =
