...
async fn test_mongodb_incremental_loadtest_small_scale() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let sync_config = surreal_sync_mongodb::SyncConfig {
    ...
    // Convert to mongodb crate's checkpoint type via string serialization
    let sync_checkpoint =
        surreal_sync_mongodb::SyncCheckpoint::from_string(&main_checkpoint.to_string())?;
