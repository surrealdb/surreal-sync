...
#[tokio::test]
async fn test_postgresql_wal_stream_only_cli() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let checkpoint_dir = format!(".test-postgresql-wal-stream-cli-checkpoints-{test_id}");
    ...
    let wal_checkpoint: surreal_sync_postgresql_wal_source::WalCheckpoint =
