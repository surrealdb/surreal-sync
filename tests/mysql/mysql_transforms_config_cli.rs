//! CLI e2e: `from mysql incremental --transforms-config` with an external mutate worker.
...
#[tokio::test]
async fn test_mysql_incremental_cli_transforms_config_mutate(
    ...
    let checkpoint_dir = format!(".test-mysql-transforms-cli-checkpoints-{test_id}");
    ...
    assert_cli_success(&output, "MySQL trigger full sync CLI");
...
"#,
    ...
    assert_cli_success(
        &incr_output,
        "MySQL trigger incremental CLI with --transforms-config mutate",
    );
