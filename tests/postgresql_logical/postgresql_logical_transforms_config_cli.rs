//! CLI e2e: `from postgresql incremental --transforms-config` with an external mutate worker.
...
#[tokio::test]
async fn test_postgresql_wal2json_incremental_cli_transforms_config_mutate(
    ...
    let checkpoint_dir = format!(".test-wal2json-transforms-cli-checkpoints-{test_id}");
    ...
    assert_cli_success(&output, "PostgreSQL wal2json full sync CLI");
...
"#,
    ...
    assert_cli_success(
        &incr_output,
        "PostgreSQL wal2json incremental CLI with --transforms-config mutate",
    );
