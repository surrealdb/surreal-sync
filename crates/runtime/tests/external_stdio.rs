...
use surreal_sync_runtime::test_support::{RecordingSink, ScriptedChangeFeed};
...
async fn write_rows_through_external_mutate() {
    ...
    let sink = RecordingSink::new();
    let opts = ApplyOpts::default();
    let row = surreal_sync_core::Row::builder("users", 0, Value::Int64(1))
        .field(
            "name",
            Value::VarChar {
                value: "alice".into(),
...
"#
    ...
    let cfg = surreal_sync_runtime::parse_transforms_toml(&toml).unwrap();
...
"#
    ...
    let cfg = surreal_sync_runtime::parse_transforms_toml(&fail_fast).unwrap();
    let pipeline = Pipeline::from_config(&cfg).unwrap();
    let opts = ApplyOpts::from_transforms_config(&cfg);
    let mut feed = ScriptedChangeFeed::new(vec![positioned(1, 10)]);
    let sink = RecordingSink::new();
...
"#
    ...
    let cfg = surreal_sync_runtime::parse_transforms_toml(&recover_then_fail).unwrap();
    assert_eq!(cfg.stages.len(), 2);
...
"#
    ...
    let cfg = surreal_sync_runtime::parse_transforms_toml(&both_ok).unwrap();
    let pipeline = Pipeline::from_config(&cfg).unwrap();
    let opts = ApplyOpts::from_transforms_config(&cfg);
