...
use surreal_sync_mysql::binlog_protocol::{
    BinlogClient, EventBody, ReplicaOptions, RowChange, SslMode,
};
...
impl surreal_sync_core::SurrealSink for MemSink {
    async fn write_rows(&self, rows: &[surreal_sync_core::Row]) -> anyhow::Result<()> {
        ...
        for row in rows {
            changes.push(format!(
                "{:?}:{}",
                surreal_sync_core::ChangeOp::Update,
                row.table
            ));
        }
        Ok(())
    ...
    async fn apply_change(&self, change: &surreal_sync_core::Change) -> anyhow::Result<()> {
        self.changes
            .lock()
            .expect("lock")
...
async fn gtid_checkpoint_resume_applies_subsequent_changes() -> Result<()> {
    ...
    let manager = surreal_sync_core::SyncManager::new(
        surreal_sync_runtime::checkpoint_fs::FilesystemStore::new(tmp.path()),
    );
    ...
    let persisted: CatchUpProgress = manager
        .read_checkpoint(surreal_sync_core::SyncPhase::CatchUpProgress)
        .await?;
    assert_ne!(
...
async fn mariadb_gtid_checkpoint_resume_applies_subsequent_changes() -> Result<()> {
    ...
    assert!(
        matches!(
            position,
            surreal_sync_mysql::binlog_protocol::BinlogPosition::MariaDbGtid { .. }
        ),
...
async fn mysql_runtime_checkpoint_is_gtid() -> Result<()> {
    ...
    assert!(
        matches!(
            position,
            surreal_sync_mysql::binlog_protocol::BinlogPosition::MySqlGtid { .. }
        ),
...
async fn sequential_full_sync_captures_real_master_position() -> Result<()> {
    ...
    use surreal_sync_mysql::from_binlog::{run_full_sync, SyncOpts};
    ...
    let tmp = tempfile::TempDir::new()?;
    let manager = SyncManager::new(surreal_sync_runtime::checkpoint_fs::FilesystemStore::new(
        tmp.path(),
    ));
...
async fn interleaved_snapshot_cancel_persists_lower_bound_and_skips_end() -> Result<()> {
    ...
    let outcome = surreal_sync_mysql::from_binlog::run_interleaved_snapshot_full_sync(
        &sink,
        &source_opts,
        8,
        cancel,
        Some(&manager),
        surreal_sync_mysql::from_binlog::InterleavedFullSyncOptions::default(),
    )
    .await?;
    ...
    let start: BinlogCheckpoint = manager
        .read_checkpoint(surreal_sync_core::SyncPhase::FullSyncStart)
        .await?;
    ...
    assert!(
        manager
            .read_checkpoint::<BinlogCheckpoint>(surreal_sync_core::SyncPhase::FullSyncEnd)
            .await
            .is_err(),
...
async fn start_at_head_checkpoint_resumes_from_current_position() -> Result<()> {
    ...
    let head = surreal_sync_mysql::from_binlog::capture_head_checkpoint(&source_opts).await?;
...
async fn steady_state_stream_honors_execute_snapshot_signal() -> Result<()> {
    use surreal_sync_mysql::from_binlog::request_snapshot;
