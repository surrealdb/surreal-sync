...
use surreal_sync_mysql::binlog_protocol::{
    BinlogClient, Flavor, ReplicaOptions, SslMode, SslOptions,
};
...
async fn tls_required_fails_when_server_cannot_use_tls() -> Result<()> {
    ...
    let pool_err =
        surreal_sync_mysql::new_mysql_pool_with_ssl(&c.connection_string, &SslMode::required())
            .await?
            .get_conn()
            .await;
    assert!(
...
async fn tls_sql_pool_modes_on_tls_server() -> Result<()> {
    ...
    // disabled
    let pool =
        surreal_sync_mysql::new_mysql_pool_with_ssl(&c.connection_string, &SslMode::Disabled)
            .await?;
    ...
    let pool =
        surreal_sync_mysql::new_mysql_pool_with_ssl(&c.connection_string, &preferred).await?;
    let mut conn = pool.get_conn().await?;
    let _: Option<i32> = conn.query_first("SELECT 1").await?;
    drop(conn);
    pool.disconnect().await?;
    ...
    let pool = surreal_sync_mysql::new_mysql_pool_with_ssl(&c.connection_string, &required).await?;
    let mut conn = pool.get_conn().await?;
    let _: Option<i32> = conn.query_first("SELECT 1").await?;
    drop(conn);
    pool.disconnect().await?;
...
async fn tls_sql_pool_preferred_does_not_fallback_on_auth_failure() -> Result<()> {
    ...
    let err = surreal_sync_mysql::new_mysql_pool_with_ssl(
        &bad_uri,
        &SslMode::Preferred(SslOptions {
            ca: Some(c.ca_path()),
            cert: None,
...
async fn tls_client_certificate_auth_connects() -> Result<()> {
    ...
    let pool = surreal_sync_mysql::new_mysql_pool_with_ssl(
        &user_uri,
        &SslMode::Required(SslOptions {
            ca: Some(c.ca_path()),
            cert: Some(c.client_cert_path()),
