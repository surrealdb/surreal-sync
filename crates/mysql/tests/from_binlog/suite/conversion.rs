...
use surreal_sync_core::{ChangeOp, ColumnDefinition, DatabaseSchema, TableDefinition, Type, Value};
use surreal_sync_mysql::binlog_protocol::{
    BinlogClient, CdcChange, EventBody, ReplicaOptions, SslMode,
};
use surreal_sync_mysql::from_binlog::cdc_to_change;
use surreal_sync_mysql::json_columns::get_json_columns;

async fn connect_client(
    conn_str: &str,
    flavor: surreal_sync_mysql::binlog_protocol::Flavor,
    server_id: u32,
) -> Result<BinlogClient> {
...
async fn wait_for_users_insert(
    ...
    CdcChange,
    surreal_sync_mysql::binlog_protocol::TableMapEvent,
)> {
