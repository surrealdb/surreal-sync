...
use crate::binlog_protocol::options::ResumePosition;
use crate::binlog_protocol::shared::wire::PacketChannel;
...
pub async fn dump_from_resume(
    ...
    match resume {
        ...
        _ => {
            crate::binlog_protocol::shared::wire::encode_binlog_dump_async(
                channel, server_id, filename, position,
            )
            .await
        }
    }
