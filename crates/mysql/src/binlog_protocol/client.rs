...
use crate::binlog_protocol::flavor::mariadb::register;
use crate::binlog_protocol::flavor::mysql::dump_gtid;
use crate::binlog_protocol::flavor::Flavor;
use crate::binlog_protocol::options::{ReplicaOptions, ResumePosition, SslMode};
...
use crate::binlog_protocol::shared::wire::{
...
enum Tracker {
    MySql(crate::binlog_protocol::flavor::mysql::MySqlPositionTracker),
    MariaDb(crate::binlog_protocol::flavor::mariadb::MariaDbPositionTracker),
}
...
impl Tracker {
    ...
    fn set_pending_mariadb_gtid(
        &mut self,
        gtid: crate::binlog_protocol::flavor::mariadb::MariaDbGtid,
    ) {
    ...
    fn set_pending_mysql_gtid(&mut self, gtid: crate::binlog_protocol::flavor::mysql::Gtid) {
    ...
    fn seed_mariadb_gtid_list(
        &mut self,
        list: crate::binlog_protocol::flavor::mariadb::MariaDbGtidList,
    ) {
        if let Tracker::MariaDb(t) = self {
    ...
    fn seed_mysql_gtid_set(&mut self, set: crate::binlog_protocol::flavor::mysql::MySqlGtidSet) {
        if let Tracker::MySql(t) = self {
            t.seed_gtid_set(set);
        }
    }
...
impl BinlogClient {
    ...
    async fn connect_once(opts: ReplicaOptions) -> Result<Self, Error> {
        ...
        let tracker = match flavor {
            Flavor::MySql => Tracker::MySql(
                crate::binlog_protocol::flavor::mysql::MySqlPositionTracker::new(
                    "mysql-bin.000001",
                    4,
                ),
            ),
            Flavor::MariaDb => Tracker::MariaDb(
                crate::binlog_protocol::flavor::mariadb::MariaDbPositionTracker::new(
                    "mysql-bin.000001",
                    4,
                ),
            ),
    ...
    fn handle_side_effects(&mut self, event: &RawEvent) {
        ...
        match event.body {
            EventBody::Gtid(crate::binlog_protocol::types::GtidMarker::MariaDb {
                domain_id,
                server_id,
                sequence,
            }) => {
                self.tracker.set_pending_mariadb_gtid(
                    crate::binlog_protocol::flavor::mariadb::MariaDbGtid {
                        domain_id,
                        server_id,
                        sequence,
                    },
            ...
            EventBody::Gtid(crate::binlog_protocol::types::GtidMarker::MySql {
                source_id,
                gno,
            }) => {
                self.tracker
                    .set_pending_mysql_gtid(crate::binlog_protocol::flavor::mysql::Gtid {
                        source_id,
                        gno,
                    });
            }
