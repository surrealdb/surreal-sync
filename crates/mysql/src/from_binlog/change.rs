...
use crate::binlog_protocol::{CdcChange, CellValue, ColumnDef, RowChange, TableMapEvent};
use crate::{
    apply_mysql_json_diffs_to_cell, binlog_cell_to_universal_value, BinlogColumnMeta,
    RowConversionConfig,
};
use anyhow::{anyhow, Result};
use surreal_sync_core::{Change, ChangeOp, DatabaseSchema, Type, Value};
...
fn table_pk_columns(table_def: &surreal_sync_core::TableDefinition) -> Vec<String> {
    table_def
...
fn binlog_column_meta(
    col_def: &ColumnDef,
    column_name: &str,
    table_def: &surreal_sync_core::TableDefinition,
