...
use surreal_sync_core::{CheckpointStore, SyncManager, SyncPhase};
...
use crate::from_binlog::schema::collect_mysql_database_schema;
use crate::from_binlog::signal::SIGNAL_TABLE;
use crate::from_binlog::{SourceOpts, SyncOpts};
...
async fn collect_schema_info(
    ...
    let json_columns_by_table = crate::json_columns::get_json_columns(conn, database).await?;
    ...
    if !pk_columns.is_empty() {
        if sync_opts.dry_run {
            ...
            loop {
                ...
                let chunk = crate::read_table_chunk(
                    conn,
                    table_name,
                    &pk_columns,
                    after.as_deref(),
...
pub async fn read_table_chunk(
    ...
    crate::read_table_chunk(conn, table_name, pk_columns, after, limit, config)
        .await
...
pub async fn capture_binlog_checkpoint(
    pool: &Pool,
    flavor: crate::binlog_protocol::Flavor,
) -> Result<BinlogCheckpoint> {
    use crate::binlog_protocol::{BinlogPosition, Flavor};
...
fn parse_mysql_gtid_executed(
    raw: Option<&str>,
) -> Result<Option<crate::binlog_protocol::BinlogPosition>> {
    use crate::binlog_protocol::{BinlogPosition, MySqlGtidSet};
...
fn parse_mariadb_gtid_binlog_pos(
    ...
    use crate::binlog_protocol::{BinlogPosition, MariaDbGtidList};
    let Some(raw) = raw else {
        return Ok(None);
    };
...
#[allow(dead_code)]
pub async fn load_schema(pool: &Pool, database: &str) -> Result<surreal_sync_core::DatabaseSchema> {
    let mut conn = pool.get_conn().await?;
    use_database(&mut conn, database).await?;
