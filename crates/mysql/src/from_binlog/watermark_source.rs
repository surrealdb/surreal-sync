...
use crate::binlog_protocol::{BinlogClient, CdcChange, EventBody, RowChange, TableMapEvent};
...
use crate::from_binlog::checkpoint::{
    get_current_checkpoint, BinlogCheckpoint, BinlogReconciliationPos,
};
...
use crate::from_binlog::full_sync::{get_primary_key_columns, read_table_chunk};
...
impl BinlogWatermarkSource {
    ...
    fn apply_table_renames(&mut self, renames: &[crate::from_binlog::ddl::TableRename]) {
    ...
    async fn refresh_schema_metadata(&mut self) -> Result<()> {
        ...
        self.json_columns =
            crate::json_columns::get_json_columns(&mut conn, &self.database).await?;
            ...
            match event.body {
                EventBody::Query(query) => {
                    if crate::from_binlog::ddl::is_table_affecting_ddl(&query, &self.database) {
                        ...
                        self.apply_table_renames(&crate::from_binlog::ddl::parse_table_renames(
                            &query.sql,
                        ));
...
fn signal_insert_to_event(
    change: &CdcChange,
    values: &[crate::binlog_protocol::CellValue],
    low: Option<Uuid>,
    high: Option<Uuid>,
...
fn signal_uuid_from_row(values: &[crate::binlog_protocol::CellValue]) -> Result<Uuid> {
    let id_cell = values
        .first()
        .ok_or_else(|| anyhow!("signal row missing id"))?;
    let id_str = match id_cell {
        crate::binlog_protocol::CellValue::String(s)
        | crate::binlog_protocol::CellValue::JsonText(s) => s.clone(),
...
pub(crate) async fn write_catch_up_for_tables<St: surreal_sync_core::CheckpointStore>(
    ...
    kind: CoverageKind,
    checkpoint: &crate::from_binlog::checkpoint::BinlogCheckpoint,
...
async fn reconciliation_pos_from_snapshot_checkpoint(
    ...
    let flavor = from_opts
        .flavor
        .unwrap_or(crate::binlog_protocol::Flavor::MySql);
    Ok(BinlogCheckpoint {
        flavor,
