...
//!
//! Canonical values live in `scripts/test-images.env` at the repo root.
//! Override at runtime with `MYSQL_BINLOG_IMAGE` / `MARIADB_BINLOG_IMAGE`.

const ENV_FILE: &str = include_str!("../../../scripts/test-images.env");
...
fn binlog_image(env_var: &str) -> String {
    std::env::var(env_var)
        ...
        .unwrap_or_else(|| panic!("{env_var} not set and missing from scripts/test-images.env"))
}
