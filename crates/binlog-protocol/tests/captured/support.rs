...
use binlog_protocol::{CellValue, EventBody, Flavor, RawEvent};
use serde::{Deserialize, Serialize};

pub const FIXTURE_DIR: &str = concat!(env!("CARGO_MANIFEST_DIR"), "/tests/fixtures/captured");
        ...
        match event.body {
            ...
            EventBody::Rows(rows) => {
                ...
                for row in rows.rows {
                    match row {
                        binlog_protocol::RowChange::Insert(cells) => out.push(SnapshotEvent {
                            kind: "Rows".into(),
                            database: None,
                        ...
                        binlog_protocol::RowChange::Update { before, after } => {
                            out.push(SnapshotEvent {
                                kind: "Rows".into(),
                                database: None,
                                table: Some(table.into()),
                        ...
                        binlog_protocol::RowChange::Delete(cells) => out.push(SnapshotEvent {
                            kind: "Rows".into(),
                            database: None,
                            table: Some(table.into()),
                            query_prefix: None,
