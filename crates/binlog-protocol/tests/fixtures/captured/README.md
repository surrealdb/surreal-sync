...
```bash
CAPTURE_BINLOG_FIXTURES=1 cargo test -p binlog-protocol capture_binlog_fixtures -- --nocapture
```
...
```bash
cargo test -p binlog-protocol replay_captured -- --nocapture
INSTA_UPDATE=1 cargo test -p binlog-protocol replay_captured
```

Normal CI and local runs use `cargo test -p binlog-protocol` only (no Docker).
