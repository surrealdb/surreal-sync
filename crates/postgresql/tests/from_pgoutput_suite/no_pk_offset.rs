//! No-PK OFFSET/LIMIT full sync (pgoutput) — proves the empty-PK branch is reachable.
...
use surreal_sync_postgresql::from_pgoutput::{run_full_sync_cancellable_with_transforms, SyncOpts};
...
async fn get_primary_key_columns_returns_empty_for_no_pk_table() -> Result<()> {
    ...
    let conn_str = crate::shared::create_test_db(container, "nopk_probe_pg").await?;
...
async fn read_offset_table_chunk_assigns_synthetic_ids_with_ctid_order() -> Result<()> {
    crate::shared::init_logging();
    let container = crate::shared::shared_postgresql_pgoutput().await;
    let conn_str = crate::shared::create_test_db(container, "nopk_chunk_pg").await?;
    let client = crate::shared::pg_connect(&conn_str).await?;
    client
        .batch_execute(
...
async fn full_sync_streams_no_pk_table_via_offset_chunks() -> Result<()> {
    crate::shared::init_logging();
    let container = crate::shared::shared_postgresql_pgoutput().await;
    let conn_str = crate::shared::create_test_db(container, "nopk_full_pg").await?;
    let client = crate::shared::pg_connect(&conn_str).await?;
    client
        .batch_execute(
            "CREATE TABLE notes (name TEXT NOT NULL); \
    ...
    // batch_size 2 forces multiple OFFSET pages through RowChunkDriver.
