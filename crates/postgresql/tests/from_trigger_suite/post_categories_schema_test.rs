...
use surreal_sync_core::{ColumnDefinition, TableDefinition, Type, Value};
...
async fn test_schema_collection_identifies_array_types() {
    ...
    // Collect schema
    let schema =
        surreal_sync_postgresql::from_trigger::schema::collect_postgresql_database_schema(&client)
            .await
...
fn test_json_array_conversion_with_schema() {
    use surreal_sync_json::types::json_to_universal_with_table_schema;
...
async fn test_complete_array_flow() {
    ...
    let result = surreal_sync_json::types::json_to_universal_with_table_schema(
