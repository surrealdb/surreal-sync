...
use surreal_sync_postgresql::from_wal2json::{
...
struct CaptureSink {
    ...
    rows: Mutex<Vec<Row>>,
    relation_changes: Mutex<Vec<surreal_sync_core::RelationChange>>,
}
...
impl SurrealSink for CaptureSink {
    ...
    async fn write_relations(
        &self,
        relations: &[surreal_sync_core::Relation],
    ) -> anyhow::Result<()> {
        let mut relation_changes = self.relation_changes.lock().expect("lock");
        for relation in relations {
            relation_changes.push(surreal_sync_core::RelationChange::update(relation.clone()));
        }
        Ok(())
    }
    ...
    async fn apply_relation_change(
        &self,
        change: &surreal_sync_core::RelationChange,
    ) -> anyhow::Result<()> {
...
async fn mem_surreal_sink() -> Result<(
    ...
    Ok((surreal_sync_surreal::v2::Surreal2Sink::new(db.clone()), db))
}
