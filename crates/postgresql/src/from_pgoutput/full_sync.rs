...
use crate::from_pgoutput::checkpoint::PgoutputCheckpoint;
use crate::from_pgoutput::client::{
    connect_wal_client, ensure_publication_for_source, get_current_wal_lsn, new_sql_client,
    resolve_schema,
};
use crate::from_pgoutput::schema::collect_postgresql_database_schema;
use crate::from_pgoutput::{SourceOpts, SyncOpts};
...
async fn migrate_table_with_transforms<S: SurrealSink>(
    ...
    use surreal_sync_core::{classify_table, TableKind, Value};
...
async fn migrate_relation_streaming<S: SurrealSink>(
    ...
    use crate::{read_offset_relation_chunk, read_relation_chunk};
    use async_trait::async_trait;
    use std::sync::Arc;
    use surreal_sync_core::{Relation, Value};
    use surreal_sync_runtime::{
...
async fn migrate_offset_rows_streaming<S: SurrealSink>(
    ...
    use crate::read_offset_table_chunk;
    use async_trait::async_trait;
    use std::sync::Arc;
    use surreal_sync_core::Row;
    use surreal_sync_runtime::{
...
pub use crate::read_table_chunk;

#[allow(dead_code)]
pub async fn load_schema(sql: &Arc<Mutex<Client>>) -> Result<surreal_sync_core::DatabaseSchema> {
    let client = sql.lock().await;
    collect_postgresql_database_schema(&client).await
}
