...
use crate::pgoutput_protocol::{Lsn, PgWalClient, RelationMeta, StreamEvent};
use anyhow::Result;
...
use crate::from_pgoutput::checkpoint::{get_current_checkpoint, PgoutputCheckpoint};
...
use crate::from_pgoutput::signal::{
    acknowledge_execute_snapshot_signal, read_pending_execute_snapshot_signals,
};
use crate::from_pgoutput::watermark_source::{
...
/// Replication tail via [`PgoutputSourceDriver`] + [`surreal_sync_runtime::run_source_runtime_with`] (shared apply window).
///
...
/// pgoutput CDC driver for [`surreal_sync_runtime::run_source_runtime_with`].
                ...
                StreamEvent::Change(change) => {
                    let rename_from_filter = self.table_filter.as_ref().and_then(|tables| {
                        if !tables.contains(&change.table) && tables.len() == 1 {
                            Some(crate::from_pgoutput::ddl::TableRename {
                                old: tables[0].clone(),
...
fn apply_renames_to_filter(
    ...
    renames: &[crate::from_pgoutput::ddl::TableRename],
) {
...
mod tests {
    use super::*;
    use crate::from_pgoutput::ddl::TableRename;
