...
use crate::new_postgresql_client;
use crate::pgoutput_protocol::{
...
pub async fn ensure_publication_for_source(
    ...
    client
        .batch_execute(&crate::from_pgoutput::signal::create_signal_table_sql())
        .await?;
    if from_opts.tables.is_empty() {
        ...
        if !tables
            .iter()
            .any(|t| t == crate::from_pgoutput::signal::SIGNAL_TABLE)
        {
            tables.push(crate::from_pgoutput::signal::SIGNAL_TABLE.to_string());
        }
...
pub async fn start_wal_from_checkpoint(
    ...
    checkpoint: &crate::from_pgoutput::PgoutputCheckpoint,
) -> Result<()> {
