...
use surreal_sync_core::{
    classify_table, Change, ChangeOp, DatabaseSchema, RelationChange, TableKind,
};
...
use surreal_sync_surreal::v2::Surreal2Store;
use tokio_postgres::NoTls;
use tracing::{debug, error, info, warn};

use crate::from_wal2json::checkpoint::PostgreSQLLogicalCheckpoint;
use crate::from_wal2json::full_sync::SourceOpts;
use crate::from_wal2json::logical_replication::{ChangeAtLsn, Slot};
use crate::from_wal2json::watermark_source::Lsn;
...
/// [`surreal_sync_runtime::run_source_runtime_with`] → advance slot only after sink
...
pub async fn run_incremental_sync_with_transforms<S: SurrealSink>(
    ...
    let pg_client = crate::from_wal2json::Client::new(client, from_opts.tables.clone());
...
/// wal2json CDC driver for [`surreal_sync_runtime::run_source_runtime_with`].
///
    ...
    fn convert_actions(
        ...
        for change in changes {
            let (row, op) = match &change.action {
                crate::from_wal2json::Action::Insert(row) => {
                    debug!(
                ...
                crate::from_wal2json::Action::Update(row) => {
                    debug!(
                ...
                crate::from_wal2json::Action::Delete(row) => {
                    debug!(
                ...
                crate::from_wal2json::Action::Begin { .. }
                | crate::from_wal2json::Action::Commit { .. } => continue,
            };
...
fn row_to_change(row: &crate::from_wal2json::Row, op: ChangeOp) -> Change {
...
fn action_to_positioned_event(
    ...
    match table_kind {
        Some(TableKind::Relation {
            ...
            let relation = crate::fk_transform::build_relation_from_change(
                &row.table,
        ...
        _ => {
            ...
            if let (Some(td), Some(ref mut data)) = (table_def, change.fields.as_mut()) {
                crate::fk_transform::transform_fk_values(data, td);
            }
