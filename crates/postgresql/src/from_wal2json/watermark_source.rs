...
//! [`WatermarkSource`](surreal_sync_runtime::WatermarkSource) trait
...
use crate::from_wal2json::change::Action;
use crate::from_wal2json::full_sync::SourceOpts;
use crate::from_wal2json::logical_replication::{Client, Slot};
...
/// [`crate::get_user_tables`] already excludes it from the
...
impl WatermarkSource for Wal2JsonWatermarkSource {
    ...
    async fn snapshot_tables(&self) -> Result<Vec<TableSpec>> {
        let tables = if self.explicit_tables.is_empty() {
            crate::get_user_tables(self.client.pg_client(), &self.schema).await?
        } else {
        ...
        for table in tables {
            ...
            let pk_columns =
                crate::get_primary_key_columns(self.client.pg_client(), &table).await?;
    ...
    async fn resolve_tables(&self, names: &[String]) -> Result<Vec<TableSpec>> {
        ...
        for name in names {
            ...
            let pk_columns = crate::get_primary_key_columns(self.client.pg_client(), name).await?;
            if pk_columns.is_empty() {
                anyhow::bail!(
