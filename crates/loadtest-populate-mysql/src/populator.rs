...
impl MySQLPopulator {
    /// Create a new MySQL populator.
    ///
    /// # Arguments
    ///
    /// * `uri` - MySQL connection URI (e.g., "mysql://root:root@localhost:3306/testdb")
    ...
    ///
    /// ```ignore
    /// let populator = MySQLPopulator::new(
    ...
    pub async fn drop_table(&self, table_name: &str) -> Result<(), MySQLPopulatorError> {
    ...
    pub async fn create_table(&self, table_name: &str) -> Result<(), MySQLPopulatorError> {
        let sql = generate_create_table(&self.schema, table_name)
            .ok_or_else(|| MySQLPopulatorError::TableNotFound(table_name.to_string()))?;
    ...
    pub async fn recreate_table(&self, table_name: &str) -> Result<(), MySQLPopulatorError> {
    ...
    pub async fn populate(
        ...
        table_name: &str,
        count: u64,
    ) -> Result<PopulateMetrics, MySQLPopulatorError> {
        ...
        while remaining > 0 {
            ...
            let inserted = insert_batch(&self.pool, &table_schema, &rows).await?;
    ...
    pub async fn truncate_table(&self, table_name: &str) -> Result<(), MySQLPopulatorError> {
        let mut conn = self.pool.get_conn().await?;
    ...
    pub async fn row_count(&self, table_name: &str) -> Result<u64, MySQLPopulatorError> {
        let mut conn = self.pool.get_conn().await?;
