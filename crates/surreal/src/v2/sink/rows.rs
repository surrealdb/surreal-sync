...
use super::write::{write_record, write_relation};
use crate::v2::types::{RecordWithSurrealValues, Relation as SurrealRelation, SurrealValue};
use anyhow::{bail, Result};
use std::collections::HashMap;
use surreal_sync_core::{Relation, Row, Value, ZeroTemporalPolicy};
use surrealdb2::sql::{Array, Id, Strand, Thing, Value as SqlValue};
...
pub fn value_to_surreal_id(value: &Value) -> Result<Id> {
    match value {
        ...
        Value::Uuid(u) => Ok(Id::Uuid(surrealdb2::sql::Uuid::from(*u))),
        Value::Ulid(u) => {
...
fn universal_value_to_id_array_element(value: &Value) -> Result<SqlValue> {
    match value {
        ...
        Value::Uuid(u) => Ok(SqlValue::Uuid(surrealdb2::sql::Uuid::from(*u))),
