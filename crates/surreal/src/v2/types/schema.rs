...
use surreal_sync_core::{DatabaseSchema, TableDefinition, Type};
use surrealdb2::sql::{Number, Strand, Value};

/// Convert JSON value to surrealdb2::sql::Value using schema information for type precision.
///
...
/// Convert JSON value to surrealdb2::sql::Value using Type for type precision.
///
    ...
    match (value, column_type) {
        ...
        (serde_json::Value::String(s), Some(Type::LocalDateTime)) => {
            ...
            if let Ok(ndt) = NaiveDateTime::parse_from_str(&s, "%Y-%m-%d %H:%M:%S") {
                ...
                Ok(Value::Datetime(surrealdb2::sql::Datetime::from(dt)))
                ...
                Ok(Value::Datetime(surrealdb2::sql::Datetime::from(
        ...
        (serde_json::Value::String(s), Some(Type::Set { .. })) => {
            if s.is_empty() {
                Ok(Value::Array(surrealdb2::sql::Array::from(
                    Vec::<Value>::new(),
                )))
            } else {
                let values: Vec<Value> = s
                    ...
                    .collect();
                Ok(Value::Array(surrealdb2::sql::Array::from(values)))
            }
        }
...
) -> anyhow::Result<surrealdb2::sql::Id> {
...
fn convert_id_with_universal_type(
    ...
    match id_type {
        Type::Int8 { .. } | Type::Int16 | Type::Int32 | Type::Int64 => {
            ...
            Ok(surrealdb2::sql::Id::Number(id_int))
        }
        Type::Uuid => {
            ...
            Ok(surrealdb2::sql::Id::Uuid(surrealdb2::sql::Uuid::from(uuid)))
        }
        Type::Text | Type::VarChar { .. } | Type::Char { .. } => {
            Ok(surrealdb2::sql::Id::String(id_str.to_string()))
        }
        other => {
            tracing::warn!(
...
/// Convert JSON value to surrealdb2::sql::Value without schema information
///
    ...
    match value {
        ...
        serde_json::Value::String(s) => {
            ...
            if let Some(duration) = try_parse_iso8601_duration(&s) {
                Value::Duration(surrealdb2::sql::Duration::from(duration))
            } else {
        ...
        serde_json::Value::Array(arr) => {
            let values: Vec<Value> = arr
                ...
                .collect();
            Value::Array(surrealdb2::sql::Array::from(values))
        }
        serde_json::Value::Object(map) => {
            ...
            Value::Object(surrealdb2::sql::Object::from(obj))
        }
    }
}
...
mod tests {
    use super::*;
    use surreal_sync_core::ColumnDefinition;
    ...
    fn test_new_convert_id_integer() {
        ...
        match id {
            surrealdb2::sql::Id::Number(n) => assert_eq!(n, 12345),
    ...
    fn test_new_convert_id_string() {
        ...
        match id {
            surrealdb2::sql::Id::String(s) => assert_eq!(s, "doc-123-abc"),
