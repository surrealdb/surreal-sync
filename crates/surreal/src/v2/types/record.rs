...
pub fn typed_values_to_record_using_field_as_id(
    ...
    id_field: &str,
    table_name: &str,
) -> anyhow::Result<super::RecordWithSurrealValues> {
...
pub fn typed_values_to_record_with_bytes_id(
    ...
    let surreal_id = surrealdb2::sql::Id::String(base64_str);
...
fn typed_values_to_record_with_id(
    typed_values: std::collections::HashMap<String, TypedValue>,
    surreal_id: surrealdb2::sql::Id,
    table_name: &str,
) -> anyhow::Result<super::RecordWithSurrealValues> {
    // Convert typed values to SurrealDB values
    let surreal_values = super::typed_values_to_surreal_map(typed_values);

    let r = super::RecordWithSurrealValues {
        id: surrealdb2::sql::Thing::from((table_name, surreal_id)),
        data: surreal_values,
    };
...
fn value_to_surreal_id(value: &Value) -> Result<surrealdb2::sql::Id> {
    match value {
