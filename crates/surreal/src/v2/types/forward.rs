...
use surrealdb2::sql::{Array, Datetime, Number, Object, Strand, Thing, Value as SqlValue};
...
impl SurrealValue {
    ...
    pub fn from_zero_temporal(
        intended_type: &Type,
        source: Option<&str>,
        policy: surreal_sync_core::ZeroTemporalPolicy,
    ) -> Self {
        match policy {
            surreal_sync_core::ZeroTemporalPolicy::None => SurrealValue(SqlValue::None),
            surreal_sync_core::ZeroTemporalPolicy::Null => SurrealValue(SqlValue::Null),
            surreal_sync_core::ZeroTemporalPolicy::String => {
                let s = source
    ...
    pub fn from_typed_with_policy(
        ...
        match (&tv.sync_type, &tv.value) {
            ...
            (Type::Json, Value::Array { elements, .. }) => {
                ...
                SurrealValue(SqlValue::Array(surrealdb2::sql::Array::from(surreal_arr)))
            }
...
impl From<TypedValue> for SurrealValue {
    fn from(tv: TypedValue) -> Self {
        Self::from_typed_with_policy(tv, surreal_sync_core::ZeroTemporalPolicy::default())
    }
}
        ...
        match value {
            ...
            Value::Char { value, .. } => {
                if let Some(duration) = try_parse_iso8601_duration(&value) {
                    SurrealValue(SqlValue::Duration(surrealdb2::sql::Duration::from(
                        duration,
                    )))
                } else {
            ...
            // Binary types
            Value::Blob(b) => SurrealValue(SqlValue::Bytes(surrealdb2::sql::Bytes::from(b))),
            Value::Bytes(b) => SurrealValue(SqlValue::Bytes(surrealdb2::sql::Bytes::from(b))),
            ...
            // UUID
            Value::Uuid(u) => SurrealValue(SqlValue::Uuid(surrealdb2::sql::Uuid::from(u))),
            ...
            Value::Duration(d) => {
                SurrealValue(SqlValue::Duration(surrealdb2::sql::Duration::from(d)))
            }
            ...
            Value::Thing { table, id } => {
                ...
                let surreal_id = match id.as_ref() {
                    ...
                    other => surrealdb2::sql::Id::String(format!("{other:?}")),
                };
                let thing = surrealdb2::sql::Thing::from((table.as_str(), surreal_id));
            ...
            Value::ZeroTemporal {
                ...
                &intended_type,
                source.as_deref(),
                surreal_sync_core::ZeroTemporalPolicy::default(),
            ),
        }
    }
}
...
/// Convert a HashMap of TypedValue to a HashMap of surrealdb2::sql::Value.
///
...
/// Record type that uses native surrealdb2::sql::Value instead of SurrealValue wrapper.
...
mod tests {
    ...
    fn test_zero_temporal_policies() {
        ...
        let none = SurrealValue::from_universal_with_policy(
            zero.clone(),
            surreal_sync_core::ZeroTemporalPolicy::None,
        );
        assert!(matches!(none.0, SqlValue::None));

        let null = SurrealValue::from_universal_with_policy(
            zero.clone(),
            surreal_sync_core::ZeroTemporalPolicy::Null,
        );
        assert!(matches!(null.0, SqlValue::Null));

        let strand = SurrealValue::from_universal_with_policy(
            zero,
            surreal_sync_core::ZeroTemporalPolicy::String,
        );
        match strand.0 {
    ...
    fn test_geometry_point_conversion() {
        ...
        let tv = TypedValue::geometry_geojson(json, surreal_sync_core::GeometryType::Point);
        let surreal_val: SurrealValue = tv.into();
