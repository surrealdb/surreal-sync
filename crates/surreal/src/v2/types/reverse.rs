...
use surrealdb2::sql::{Number, Object, Value as SqlValue};
        ...
        match (&sv.sync_type, &sv.value) {
            ...
            (Type::Thing, SqlValue::Thing(thing)) => {
                ...
                let id = match &thing.id {
                    surrealdb2::sql::Id::String(s) => Value::Text(s.clone()),
                    surrealdb2::sql::Id::Number(n) => Value::Int64(*n),
                    surrealdb2::sql::Id::Uuid(u) => Value::Uuid(u.0),
...
fn geometry_to_serde_json(geo: &surrealdb2::sql::Geometry) -> serde_json::Value {
    match geo {
        surrealdb2::sql::Geometry::Point(p) => {
            serde_json::json!({
                "type": "Point",
                "coordinates": [p.x(), p.y()]
            })
        }
        surrealdb2::sql::Geometry::Line(line) => {
        ...
        surrealdb2::sql::Geometry::Polygon(poly) => {
        ...
        surrealdb2::sql::Geometry::MultiPoint(mp) => {
...
mod tests {
    ...
    use surrealdb2::sql::{Array, Datetime, Strand};
    ...
    fn test_bytes_conversion() {
        let bytes = surrealdb2::sql::Bytes::from(vec![0x01, 0x02, 0x03]);
    ...
    fn test_uuid_conversion() {
        ...
        let sv = SurrealValueWithSchema::new(
            SqlValue::Uuid(surrealdb2::sql::Uuid::from(uuid)),
            Type::Uuid,
        );
        let tv = TypedValue::from(sv);
