//! SurrealDB v3 test client implementation.
//!
//! Provides an implementation of `SurrealTestClient` for SurrealDB v3 servers.

#[cfg(feature = "reqwest")]
use crate::client::SurrealTestClient;
use anyhow::Result;
use surrealdb3::engine::any::Any;
use surrealdb3::Surreal;

/// SurrealDB v3 test client wrapper.
pub struct Surreal3Client {
    client: Surreal<Any>,
}
...
impl Surreal3Client {
    ...
    /// Connect to SurrealDB v3 and return a wrapped client.
    pub async fn connect(
        ...
        tracing::debug!("Connecting to SurrealDB v3 at {}", ws_endpoint);

        let client = surrealdb3::engine::any::connect(&ws_endpoint).await?;

        // SurrealDB v3 requires owned Strings for auth
        client
...
impl SurrealTestClient for Surreal3Client {
...
/// Helper function to connect to SurrealDB v3.
pub async fn connect(
    endpoint: &str,
    username: &str,
    password: &str,
    namespace: &str,
    database: &str,
) -> Result<Surreal3Client> {
    Surreal3Client::connect(endpoint, username, password, namespace, database).await
}
...
mod tests {
    ...
    fn test_client_creation() {
        ...
        assert_send_sync::<Surreal3Client>();
    }
}
