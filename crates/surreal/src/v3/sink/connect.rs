...
pub async fn surreal_connect_with_retries(
    ...
    tracing::debug!(
        "Connecting to SurrealDB v3 at {} (namespace: {}, database: {})",
        surreal_endpoint,
        ns,
        db
    );
    ...
    for attempt in 1..=max_retries {
        match try_connect(&surreal_endpoint, opts, ns, db).await {
            Ok(surreal) => {
                if attempt > 1 {
                    tracing::info!(
                        "Successfully connected to SurrealDB v3 after {} attempts",
                        attempt
                    );
                }
                return Ok(surreal);
            ...
            Err(e) => {
                ...
                if attempt < max_retries {
                    tracing::warn!(
                        "Failed to connect to SurrealDB v3 at '{}' (attempt {}/{}): {}. Retrying in {}s...",
                        surreal_endpoint,
                        attempt,
                        max_retries,
    ...
    Err(anyhow::anyhow!(
        "Failed to connect to SurrealDB v3 at '{}' after {} attempts. Last error: {}",
        surreal_endpoint,
        max_retries,
        last_error.unwrap()
    ))
...
async fn try_connect(
    ...
    let surreal = surrealdb3::engine::any::connect(endpoint)
        .await
        .map_err(|e| anyhow::anyhow!("SurrealDB v3 connection to '{endpoint}' failed: {e}"))?;

    // Sign in
    let username = opts.surreal_username.clone();
    surreal
        ...
        .await
        .map_err(|e| {
            anyhow::anyhow!("SurrealDB v3 authentication failed (user: '{username}'): {e}")
        })?;

    // Select namespace and database
    surreal.use_ns(ns).use_db(db).await.map_err(|e| {
        anyhow::anyhow!("SurrealDB v3 failed to select namespace '{ns}' / database '{db}': {e}")
    })?;
