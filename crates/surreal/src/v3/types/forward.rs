...
use surrealdb3::types::{Array, Datetime, Number, Object, RecordId, RecordIdKey, Value as DbValue};
...
impl SurrealValue {
    ...
    pub fn from_zero_temporal(
        ...
        match policy {
            surreal_sync_core::ZeroTemporalPolicy::None => SurrealValue(DbValue::None),
            surreal_sync_core::ZeroTemporalPolicy::Null => SurrealValue(DbValue::Null),
        ...
        match value {
            ...
            Value::Char { value, .. } => {
                if let Some(duration) = try_parse_iso8601_duration(&value) {
                    SurrealValue(DbValue::Duration(surrealdb3::types::Duration::from(
                        duration,
                    )))
                } else {
            ...
            // Binary types
            Value::Blob(b) => SurrealValue(DbValue::Bytes(surrealdb3::types::Bytes::from(b))),
            Value::Bytes(b) => SurrealValue(DbValue::Bytes(surrealdb3::types::Bytes::from(b))),
            ...
            // UUID
            Value::Uuid(u) => SurrealValue(DbValue::Uuid(surrealdb3::types::Uuid::from(u))),
            ...
            Value::Duration(d) => {
                SurrealValue(DbValue::Duration(surrealdb3::types::Duration::from(d)))
            }
...
/// Convert a HashMap of TypedValue to a HashMap of surrealdb3::types::Value.
///
...
/// Record type that uses native surrealdb3::types::Value instead of SurrealValue wrapper.
...
mod tests {
    ...
    fn test_zero_temporal_policies() {
        ...
        let string = SurrealValue::from_universal_with_policy(
            zero,
            surreal_sync_core::ZeroTemporalPolicy::String,
        );
        match string.0 {
