...
use surrealdb3::types::{Number, RecordIdKey, Value};

/// Convert JSON value to surrealdb3::types::Value using schema information for type precision.
///
...
/// Convert JSON value to surrealdb3::types::Value using Type for type precision.
///
    ...
    match (value, column_type) {
        ...
        (serde_json::Value::String(s), Some(Type::LocalDateTime)) => {
            ...
            if let Ok(ndt) = NaiveDateTime::parse_from_str(&s, "%Y-%m-%d %H:%M:%S") {
                ...
                Ok(Value::Datetime(surrealdb3::types::Datetime::from(dt)))
                ...
                Ok(Value::Datetime(surrealdb3::types::Datetime::from(
        ...
        (serde_json::Value::String(s), Some(Type::Set { .. })) => {
            if s.is_empty() {
                Ok(Value::Array(surrealdb3::types::Array::from(
                    Vec::<Value>::new(),
                )))
            } else {
                let values: Vec<Value> =
                    s.split(',').map(|v| Value::String(v.to_string())).collect();
                Ok(Value::Array(surrealdb3::types::Array::from(values)))
            }
        }
...
fn convert_id_with_universal_type(
    ...
    match id_type {
        ...
        Type::Uuid => {
            ...
            Ok(RecordIdKey::Uuid(surrealdb3::types::Uuid::from(uuid)))
        }
...
/// Convert JSON value to surrealdb3::types::Value without schema information
///
    ...
    match value {
        ...
        serde_json::Value::String(s) => {
            ...
            if let Some(duration) = try_parse_iso8601_duration(&s) {
                Value::Duration(surrealdb3::types::Duration::from(duration))
            } else {
                Value::String(s)
            }
        }
        serde_json::Value::Array(arr) => {
            let values: Vec<Value> = arr
                ...
                .collect();
            Value::Array(surrealdb3::types::Array::from(values))
        }
        serde_json::Value::Object(map) => {
            ...
            Value::Object(surrealdb3::types::Object::from(obj))
        }
    }
}
