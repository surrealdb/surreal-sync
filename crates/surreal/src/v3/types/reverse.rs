...
use surrealdb3::types::{Number, Object, RecordIdKey, Value as DbValue};
...
fn geometry_to_serde_json(geo: &surrealdb3::types::Geometry) -> serde_json::Value {
    match geo {
        surrealdb3::types::Geometry::Point(p) => {
            serde_json::json!({
                "type": "Point",
                "coordinates": [p.x(), p.y()]
            })
        }
        surrealdb3::types::Geometry::Line(line) => {
        ...
        surrealdb3::types::Geometry::Polygon(poly) => {
        ...
        surrealdb3::types::Geometry::MultiPoint(mp) => {
...
mod tests {
    ...
    use surrealdb3::types::{Array, Datetime};
    ...
    fn test_bytes_conversion() {
        let bytes = surrealdb3::types::Bytes::from(vec![0x01, 0x02, 0x03]);
    ...
    fn test_uuid_conversion() {
        ...
        let sv = SurrealValueWithSchema::new(
            DbValue::Uuid(surrealdb3::types::Uuid::from(uuid)),
            Type::Uuid,
        );
        let tv = TypedValue::from(sv);
