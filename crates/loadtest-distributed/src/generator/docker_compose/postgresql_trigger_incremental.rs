...
//!   - Full sync setup: `from postgresql-trigger full --checkpoints-surreal-table ...`
//!   - Incremental sync: `from postgresql-trigger incremental --checkpoints-surreal-table ...`
//!
//! ## Pipeline
//!
...
/// Generate schema-init service for PostgreSQL Trigger Incremental (creates empty tables).
///
...
/// Generate full-sync-setup service for PostgreSQL Trigger Incremental.
///
...
pub fn generate_full_sync_setup_service(config: &ClusterConfig) -> Value {
    ...
    // Note: Uses postgresql-trigger (not postgresql) to set up triggers
    let tables = get_all_tables(config);
    let tables_arg = tables.join(",");

    let command = format!(
        "from postgresql-trigger full --connection-string 'postgresql://postgres:postgres@postgresql:5432/{}' --tables '{}' --checkpoints-surreal-table surreal_sync_checkpoints --to-namespace {} --to-database {} --surreal-endpoint 'http://surrealdb:8000' --surreal-username root --surreal-password root",
...
/// Generate incremental-sync service for PostgreSQL Trigger Incremental.
///
...
pub fn generate_incremental_sync_service(config: &ClusterConfig) -> Value {
    ...
    let command = format!(
        "from postgresql-trigger incremental --connection-string 'postgresql://postgres:postgres@postgresql:5432/{}' --tables '{}' --checkpoints-surreal-table surreal_sync_checkpoints --timeout 60 --to-namespace {} --to-database {} --surreal-endpoint 'http://surrealdb:8000' --surreal-username root --surreal-password root",
        config.database.database_name,
        tables_arg,
        config.surrealdb.namespace,
        config.surrealdb.database
