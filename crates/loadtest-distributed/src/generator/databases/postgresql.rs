//! PostgreSQL Docker and Kubernetes configurations for load testing.
...
/// Generate PostgreSQL Kubernetes StatefulSet.
pub fn generate_postgresql_k8s_statefulset(config: &DatabaseConfig, namespace: &str) -> String {
...
/// Generate PostgreSQL Kubernetes Service.
pub fn generate_postgresql_k8s_service(namespace: &str) -> String {
    format!(
        r#"apiVersion: v1
kind: Service
metadata:
...
mod tests {
    ...
    #[test]
    fn test_generate_postgresql_docker_service() {
        let config = test_config();
        let service = generate_postgresql_docker_service(&config);
    ...
    #[test]
    fn test_generate_postgresql_k8s_statefulset() {
        let config = test_config();
        let statefulset = generate_postgresql_k8s_statefulset(&config, "loadtest");

        assert!(statefulset.contains("kind: StatefulSet"));
        assert!(statefulset.contains("namespace: loadtest"));
        assert!(statefulset.contains("postgres:16"));
    }
}
