...
use surreal_sync_json::from_jsonl::{sync, Config, ConversionRule};
...
async fn test_jsonl_migration_e2e() -> Result<(), Box<dyn std::error::Error>> {
    tracing_subscriber::fmt()
        .with_env_filter("surreal_sync_json::from_jsonl=debug,test=debug")
        .try_init()
        .ok();
    ...
    let config = Config {
        sources: vec![surreal_sync_json::from_jsonl::FileSource::Local(
            PathBuf::from(format!("{}/", test_data_dir.display())),
        )],
        files: vec![],
        s3_uris: vec![],
        http_uris: vec![],
    ...
    match db.detected_version {
        Some(SurrealMajorVersion::V3) => {
            ...
            let sink = surreal_sync_surreal::v3::Surreal3Sink::new(surreal.clone());
        ...
        _ => {
            ...
            let sink = surreal_sync_surreal::v2::Surreal2Sink::new(surreal.clone());
            sync(&sink, config).await?;
...
async fn test_jsonl_with_custom_id_field() -> Result<(), Box<dyn std::error::Error>> {
    tracing_subscriber::fmt()
        .with_env_filter("surreal_sync_json::from_jsonl=debug")
        .try_init()
        .ok();
...
{"item_id": "item2", "name": "Test Item 2", "value": 200}"#,
    ...
    let config = Config {
        sources: vec![surreal_sync_json::from_jsonl::FileSource::Local(
            PathBuf::from(format!("{}/", test_dir.display())),
        )],
        files: vec![],
        s3_uris: vec![],
        http_uris: vec![],
