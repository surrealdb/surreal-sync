...
use crate::full_sync::read_table_chunk;
...
async fn get_snapshot_tables(
    ...
    if schema == "public" {
        return surreal_sync_postgresql::get_user_tables(client, schema).await;
    }
    let rows = client
        .query(
...
pub async fn request_snapshot(from_opts: &SourceOpts, tables: &[String]) -> Result<()> {
    let sql = new_sql_client(&from_opts.connection_string).await?;
    let client = sql.lock().await;
    crate::signal::request_snapshot(&client, tables).await
}
