...
impl From<FlavorArg> for surreal_sync_mysql_binlog_source::Flavor {
...
impl TlsArgs {
    pub fn ssl_mode(&self) -> surreal_sync_mysql_binlog_source::SslMode {
        ...
        match self.tls_mode {
            TlsModeArg::Disabled => surreal_sync_mysql_binlog_source::SslMode::Disabled,
            TlsModeArg::Preferred => surreal_sync_mysql_binlog_source::SslMode::Preferred(options),
            TlsModeArg::Required => surreal_sync_mysql_binlog_source::SslMode::Required(options),
        }
    }

    /// Map to the trigger-source SSL mode (same PEM paths / mode enum).
    pub fn trigger_ssl_mode(&self) -> surreal_sync_mysql_trigger_source::SslMode {
        match self.ssl_mode() {
