//! BigQuery CLI glue (Surreal version auto-detect).
//!
//! Embedders should use `surreal_sync_bigquery::run` with one sink type. This
//! module is CLI-only and auto-detects SurrealDB v2 vs v3.

use surreal_sync_runtime::SinkConnect;

pub use surreal_sync_bigquery::from_bigquery::cli::Args;
...
pub async fn run_args(args: Args) -> anyhow::Result<()> {
    // Reject bad BigQuery configuration before dialling SurrealDB, so a missing
    // service-account key is not reported as a sink connection failure.
    surreal_sync_bigquery::from_bigquery::cli::preflight(&args)?;
    ...
    match sdk_version {
        SdkVersion::V2 => {
            ...
            surreal_sync_bigquery::from_bigquery::cli::run_args_with_sink(args, &sink).await
        }
        SdkVersion::V3 => {
            let config = args
                .surreal
